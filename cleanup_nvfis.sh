...
curl -k -v -u $NFVIS_LOGIN_INPUT:$NFVIS_PASSWRD_INPUT -X DELETE $NFVIS_URL_INPUT/api/config/esc_datamodel/tenants/tenant/admin/deployments/deployment/ASAv
echo
echo "Moving ISRv back to LAN Network"
curl -k -v -u $NFVIS_LOGIN_INPUT:$NFVIS_PASSWRD_INPUT -X PUT $NFVIS_URL_INPUT/api/config/esc_datamodel/tenants/tenant/admin/deployments/deployment/1471415647/vm_group/ROUTER/interfaces -d @network_map.json
echo
